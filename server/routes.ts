...
function analyzeRubric(text: string): RubricAnalysisResult {
  // Basic splitting by lines and looking for numbered items
  const lines = text.split('\n').filter(line => line.trim() !== '');
  const deliverables: RubricAnalysisResult['deliverables'] = [];
...
function breakdownDeliverableIntoTasks(deliverableName: string): string[] {
